...
export class TreeChecklistExample {
  ...
  constructor(private database: ChecklistDatabase) {
    ...
    // console.log(left);
    this.diffString = JSON.stringify(this.diffs);
  }
  ...
  diffColor(diffs) {
    let currentTree = Object.assign({}, this.dataTree);
    console.log(currentTree);
    console.log("-----");
    diffs.forEach((diff, index) => {
      let pivote = currentTree;
      // console.log(diff);
      // diff.path1.path.split("/").forEach((paths) => {
      //   console.log("s", pivote, paths);
      //   pivote = pivote[paths.toString()];
      // });

      let parents = diff.path1.path.split("/");
      for (let index = 1; index < parents.length; ++index) {
        let propertyPath = parents[index];
        // console.log("s", pivote, propertyPath);
        pivote = pivote[propertyPath];
      }
      pivote += "s";
      console.log(pivote);
      console.log(currentTree);
      // let linesLeft = $(`#${blockLeftId}`)
      //   .text()
      //   .split("\n");
      // let textLeft = $(`#${blockLeftId}`).text();
      // for (let j = 0; j < linesLeft.length; j++) {
      //   if (
      //     (this.diffs[i]["path1"]["line"] == j + 1 && this.diffs[i]["type"] == "eq") ||
      //     this.diffs[i]["type"] == "type"
      //   ) {
      //     $(`#${blockLeftId}`).text(
      //       textLeft.replace(
      //         linesLeft[j],
      //         `<span style="color:#e67e22;">${linesLeft[j]}</span>`
      //       )
      //     );
      //   } else if (
      //     this.diffs[i]["path1"]["line"] == j + 1 &&
      //     this.diffs[i]["type"] == "right"
      //   ) {
      //     $(`#${blockLeftId}`).text(
      //       textLeft.replace(
      //         linesLeft[j],
      //         `<span style="color:#c0392b;">${linesLeft[j]}</span>`
      //       )
      //     );
      //   }
    });

    // let linesRight = $(`#${blockRightId}`)
    //   .text()
    //   .split("\n");
    // let textRight = $(`#${blockRightId}`).text();
    // for (let j = 0; j < linesRight.length; j++) {
    //   if (this.diffs[i]["path2"]["line"] == j + 1 && this.diffs[i]["type"] == "eq") {
    //     // $(`#${blockRightId}`).text(
    //     //   textRight.replace(
    //     //     linesRight[j],
    //     //     `<span style="color:#e67e22;">${linesRight[j]}</span>`
    //     //   )
    //     );
    //   } else if (
    //     this.diffs[i]["path2"]["line"] == j + 1 &&
    //     this.diffs[i]["type"] == "left"
    //   ) {
    //     // $(`#${blockRightId}`).text(
    //     //   textRight.replace(
    //     //     linesRight[j],
    //     //     `<span style="color:#27ae60;">${linesRight[j]}</span>`
    //     //   )
    //     // );
    //   }
    // }
